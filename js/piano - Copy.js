...
document.body.addEventListener("mousedown", async ()=>{ await Tone.start(); });
...
function buildKeyboard(){
  ...
  allNotes.forEach(note => {
    if (!isBlack(note)) {
      ...
      key.addEventListener("mousedown", e=>{ e.stopPropagation(); key.classList.add("active"); playNoteStart(note); });
      key.addEventListener("mouseup", e=>{ e.stopPropagation(); key.classList.remove("active"); playNoteStop(note); });
  ...
  allNotes.forEach(note => {
    if (isBlack(note)) {
      ...
      if (noteInOctave === 1) leftWhiteNoteMidi = note - 1;
      else if (noteInOctave === 3) leftWhiteNoteMidi = note - 1;
      else if (noteInOctave === 6) leftWhiteNoteMidi = note - 1;
      else if (noteInOctave === 8) leftWhiteNoteMidi = note - 1;
      else if (noteInOctave === 10) leftWhiteNoteMidi = note - 1;
...
window.addEventListener("mouseup", ()=>{ activeNotes.forEach(n=>keys[n]?.classList.remove("active")); });
window.addEventListener("mouseleave", ()=>{ activeNotes.forEach(n=>keys[n]?.classList.remove("active")); });
...
function setupKeyboard(){
  document.addEventListener('keydown', e=>{
    ...
    if(keyMapBase[key] !== undefined && !pressedKeys[key]){
      let note = keyMapBase[key] + octaveShift;
      if(note < 21) note = 21;
...
function setupMIDI(){
  if(navigator.requestMIDIAccess){
    ...
    }).catch(err => { console.error("Could not get access to MIDI devices:", err); });
  } else {
    console.warn("Your browser does not support MIDI API.");
  }
}
